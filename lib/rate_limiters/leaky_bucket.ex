defmodule RateLimitedClient.RateLimiters.LeakyBucket do
  use GenServer
  require Logger
  ...
  def handle_cast({:enqueue_request, request_handler, response_handler}, state) do
    ...
    Logger.info("\nEnqueuing task with req/res handlers:")
    IO.inspect(request_handler)
    IO.inspect(response_handler)
    Logger.info("current queue size: #{state.request_queue_size}")
  ...
  def handle_info(:pop_from_request_queue, state) do
    ...
    Logger.info("Request started at #{NaiveDateTime.utc_now()} UTC")
    ...
    Task.Supervisor.async_nolink(RateLimiter.TaskSupervisor, fn ->
      ...
      Logger.info("Request completed at #{NaiveDateTime.utc_now()} UTC")
    end)
